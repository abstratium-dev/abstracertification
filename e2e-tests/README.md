...
- **Set to `http://localhost:8085`**: Playwright will execute `start-e2e-server.sh` to start the server
...
- `baseURL`: Defaults to `http://localhost:8085`
...
- Set `BASE_URL=http://localhost:8085` to let Playwright start the server
