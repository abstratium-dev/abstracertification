...
INSERT INTO T_certification_step (id, certification_id, step_key, title, why, info_expanded, created_at, updated_at)
VALUES ('a1c2d3e4-f5a6-4b7c-8d9e-0f1a2b3c4d5e', 'linux-home-server', 'ssh-connect-windows', 'Connecting to the Server Using SSH — From Windows', 'Now that you have configured the SSH daemon on the server, you need to connect to it from your local machine. On Windows, the built-in OpenSSH client (available since Windows 10) is the simplest option. You will also learn to use SCP (Secure Copy Protocol) to transfer files between your local machine and the server — essential for deploying configuration files, web content, and backups.', FALSE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
...
INSERT INTO T_instruction (id, step_id, text, command, note, mermaid_diagram, sequence_order)
VALUES ('bd0e2c1c-a61d-4f20-9b35-48a33e3311cf', '9b67ca79-603b-4d2d-8bdb-af8bef07b388', 'Security is layered: the firewall blocks unwanted traffic at the network level, SSH key authentication prevents password-based attacks, and running as a non-root user limits damage if somehow compromised.', NULL, NULL, NULL, 3);
...
INSERT INTO T_instruction (id, step_id, text, command, note, mermaid_diagram, sequence_order)
VALUES ('9c3d4e5f-6a7b-8c9d-0e1f-2a3b4c5d6e7f', 'dab542a6-77df-4032-b273-d9f7fa027993', 'Open Rufus. It will automatically detect your USB drive. Under ''Boot selection'', click ''SELECT'' and browse to the Ubuntu Server ISO you downloaded.', NULL, NULL, NULL, 3);
...
INSERT INTO T_instruction (id, step_id, text, command, note, mermaid_diagram, sequence_order)
VALUES ('f601c899-95a9-416c-b173-741d9feeb95c', 'dab542a6-77df-4032-b273-d9f7fa027993', 'Update the system to ensure all packages are current.', 'sudo apt update && sudo apt upgrade -y', NULL, NULL, 11);
...
INSERT INTO T_instruction (id, step_id, text, command, note, mermaid_diagram, sequence_order)
VALUES ('b24764db-430e-4dba-ae43-6076b74ad60b', 'f6676877-8b20-4558-9b3b-91c6ffa7e1d6', 'Identify your USB device. List all block devices and find your USB drive — it is typically /dev/sdb or /dev/sdc. Do NOT select your system disk.', 'lsblk', 'Look for the device that matches the size of your USB drive. Never use /dev/sda if that is your system disk.', NULL, 2);
...
INSERT INTO T_instruction (id, step_id, text, command, note, mermaid_diagram, sequence_order)
VALUES ('00f6169e-b617-4dea-b6c8-7af3a6bad46b', 'f6676877-8b20-4558-9b3b-91c6ffa7e1d6', 'Update the system to ensure all packages are current.', 'sudo apt update && sudo apt upgrade -y', NULL, NULL, 11);
